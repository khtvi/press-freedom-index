...
def find_data_file() -> Path | None:
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .masthead-top{display:flex;align-items:center;justify-content:space-between;padding:.55rem 1.7rem;border-bottom:1px solid rgba(255,255,255,.12)}
    .masthead-title{font-family:var(--display);font-size:1.5rem;font-weight:900;letter-spacing:-.01em;line-height:1}
    ...
    .ticker{background:var(--red);color:#fff;font-family:var(--mono);font-size:9px;font-weight:600;letter-spacing:.05em;padding:.25rem 1.7rem;display:flex;gap:2rem;overflow:hidden}
    ...
    .layout{display:grid;grid-template-columns:260px 1fr;min-height:calc(100vh - 84px);height:calc(100vh - 84px);overflow:hidden}
    .sidebar{background:var(--ink2);color:var(--paper);padding:1.1rem 1rem;display:flex;flex-direction:column;gap:.9rem;border-right:2px solid var(--red);overflow:hidden}
    ...
    .main{padding:0.8rem 1rem;display:flex;flex-direction:column;gap:.7rem;overflow:hidden}
    ...
    .metric-cell{background:var(--paper2);padding:8px 10px;position:relative;cursor:pointer;transition:background .15s}
    ...
    .m-value{font-family:var(--display);font-size:1.2rem;font-weight:700;color:var(--ink);line-height:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    ...
    .section-label{display:flex;align-items:baseline;gap:10px;border-top:3px solid var(--ink);padding-top:6px;margin-top:2px}
    ...
    .section-title{font-family:var(--display);font-size:1.05rem;font-weight:700;color:var(--ink)}
    .section-sub{font-family:var(--mono);font-size:9px;color:var(--wire);margin-left:auto;letter-spacing:.05em;white-space:nowrap}
    .card{background:#fff;border:1px solid rgba(0,0,0,.12);border-radius:2px;padding:.8rem;box-shadow:3px 3px 0 rgba(0,0,0,.06)}
    ...
    .canvas-holder{position:relative;height:120px;width:100%}
    .canvas-holder.tall{height:140px}
    </style>
    </head>
    <body>
    ...
    function buildMap(){
      ...
      const locations = mapRows.map(r => ISO_COL ? r.iso : r.country).filter(Boolean);
      const scores = mapRows.map(r => r.score);
      const hover = mapRows.map(r => `${r.country}<br>Score: ${formatNumber(r.score)}`);
      const data = [{
        type: 'choropleth',
        locations: locations,
        z: scores,
    ...
    YEAR_NUMBERS = YEARS.slice().reverse();
    YEAR_NUMBERS.forEach(y => {
